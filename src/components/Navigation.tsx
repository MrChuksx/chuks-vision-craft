...
const Navigation = () => {
  ...
  return <>
      ...
      backdropFilter: "blur(12px)"
        ...
        duration: 0.8,
        delay: 0.4
      }} onClick={() => setMenuOpen(!menuOpen)} className="md:hidden relative z-[60] w-10 h-10 flex flex-col items-center justify-center gap-1.5" aria-label="Toggle menu">
          <motion.span animate={menuOpen ? {
          rotate: 45,
          y: 5
        } : {
          rotate: 0,
      ...
      <AnimatePresence>
        ...
        ease: [0.16, 1, 0.3, 1]
            ...
            <nav className="flex flex-col items-center gap-10">
              {navItems.map((item, i) => <motion.button key={item.id} initial={{
            opacity: 0,
            y: 30
          }} animate={{
            opacity: 1,
            ...
            delay: 0.1 + i * 0.08,
            ease: [0.16, 1, 0.3, 1]
          }} onClick={() => scrollTo(item.id)} className="font-display text-3xl font-semibold text-foreground hover:text-gradient-gold transition-all duration-300 tracking-wide uppercase">
                  {item.label}
                </motion.button>)}
            </nav>
          </motion.div>}
